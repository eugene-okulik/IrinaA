...
cursor.executemany(
    insert_lessons_query, [
        ('Lesson 3 Python', subject_ids[0]),
        ('Lesson 4 Python', subject_ids[0]),
        ('Lesson 3 SQL', subject_ids[1]),
        ('Lesson 4 SQL', subject_ids[1]),
    ]
)
db.commit()
...
select_query = '''
SELECT
value FROM marks WHERE student_id = %s
'''
cursor.execute(select_query, (added_student_id,))
print(cursor.fetchall())

select_query_2 = '''
SELECT title FROM books WHERE taken_by_student_id = %s
'''
cursor.execute(select_query_2, (added_student_id,))
print(cursor.fetchall())
