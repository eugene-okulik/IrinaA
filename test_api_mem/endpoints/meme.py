...
class Meme:
    ...
    def get_meme(self, meme_id):
        response = requests.get(f"{self.base_url}/meme/{meme_id}", headers=self.headers)
        print(f"GET /meme/{meme_id} response: {response.json()}")
        response.raise_for_status()
        return response.json()

    def create_meme(self, meme_data):
        response = requests.post(f"{self.base_url}/meme", json=meme_data, headers=self.headers)
        print(f"POST /meme response: {response.json()}")
        response.raise_for_status()
        return response.json()

    def update_meme(self, meme_id, meme_data):
        response = requests.put(f"{self.base_url}/meme/{meme_id}", json=meme_data, headers=self.headers)
        print(f"PUT /meme/{meme_id} response: {response.json()}")
        response.raise_for_status()
        return response.json()

    def delete_meme(self, meme_id):
        response = requests.delete(f"{self.base_url}/meme/{meme_id}", headers=self.headers)
